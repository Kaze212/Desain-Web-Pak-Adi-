...
function tampilkanDaftarBuku() {
    ...
    daftarBuku.forEach((buku, index) => {
        ...
        divBuku.classList.add('buku', 'p-4', 'border-b', 'border-gray-300', 'last:border-0');
        divBuku.innerHTML = `
            <p>${buku.tampilkanInfo()}</p>
            <button onclick="tambahKeFavorit(${index})"class="text-blue-500 mt-2">Tambah ke Favorit</button>
        `;
...
function tampilkanBukuFavorit() {
    ...
    bukuFavorit.forEach((buku, index) => {
        ...
        divBuku.innerHTML = `
            <p>${buku.tampilkanInfo()}</p>
            <button onclick="hapusDariFavorit(${index})"class="text-red-500 mt-2">Hapus</button>
        `;
...
window.onload = function () {
    ...
    // Menampilkan nama pengguna dari Session Storage
    tampilkanNamaPengguna();
>>>>>>> fe42511deb1bf92150238b07620602a416bba4a8
};